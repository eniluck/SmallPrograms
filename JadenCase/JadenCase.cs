...
namespace JadenCase
{
    ...
    public static class JadenCase
    {
        ...
        {
            bool nextCharCapitalized = true;
            string jadenCaseString = "";
            for (int i = 0; i < phrase.Length; i++)
            {
                char nextChar = phrase[i];

                if (nextCharCapitalized)
                {
                    nextChar = Char.ToUpper(nextChar);
                    nextCharCapitalized = false;
                }

                if (nextChar == ' ')
                    nextCharCapitalized = true;

                jadenCaseString = jadenCaseString + nextChar;
            }
